...
def parse_args():
    ...
    parser.add_argument(
        "--exclude", type=str, default="social",
    ...
    for behav in label_order:
        ...
        for fi, (_, trow) in enumerate(sampled.iterrows()):
            ...
            for mode in CROP_MODES:
                origin = union_origin if mode in ("union512", "darken512", "roi512") else None
                crop_np, _ = crop_frame(
                    frame_np, bbox, mode, union_origin=origin,
                )
                if crop_np is None:
                    continue
            ...
            logger.info(f"  frame {fi} (idx={frame_idx}): saved {len(CROP_MODES)} crop modes")
